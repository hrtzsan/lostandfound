...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update = $pdo->prepare("UPDATE lost_items SET item_name=?, description=?, lost_date=?, lost_place=?, contact_info=?, status=?, image=? WHERE id=?");
    $update->execute([$item_name, $description, $lost_date, $lost_place, $contact_info, $status, $image, $id]);
...
<body>
  <div class="container">
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <label>สถานที่</label>
      <input type="text" name="lost_place" value="<?= htmlspecialchars($item['lost_place']) ?>" required>
      ...
      <select name="status" required>
        <option value="ยังไม่เจอ" <?= $item['status']=="ยังไม่เจอ" ? "selected" : "" ?>>ยังไม่เจอ</option>
        <option value="เจอแล้ว" <?= $item['status']=="เจอแล้ว" ? "selected" : "" ?>>เจอแล้ว</option>
      </select>
