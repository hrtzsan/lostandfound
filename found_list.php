...
try {
    $stmt = $pdo->query("SELECT * FROM found_items ORDER BY created_at DESC");
    $found_items = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $total = count($found_items);
...
<tr>
    ...
    <td><?= htmlspecialchars($item['found_date']) ?></td>
    <td><?= htmlspecialchars($item['found_place']) ?></td>
    ...
    <td>
        <a href="found_edit.php?id=<?= $item['id'] ?>" class="btn btn-edit">แก้ไข</a>
        <a href="found_delete.php?id=<?= $item['id'] ?>" class="btn btn-delete" onclick="return confirm('คุณแน่ใจว่าจะลบรายการนี้?');">ลบ</a>
    </td>
</tr>
<?php endforeach; ?>
