...
<style>
body { font-family: 'Poppins', sans-serif; margin:0; padding:0; background:#000; color:#e0e0e0; }
...
.edit-button, .delete-button { display:inline-block; padding:8px 15px; font-size:14px; font-weight:500; border-radius:8px; margin-bottom:20px; text-decoration:none; transition:0.2s; }
.edit-button { background-color:#0071e3; color:white; }
.edit-button:hover { background-color:#147ce5; }
.delete-button { background-color:#e33030; color:white; }
.delete-button:hover { background-color:#ff4d4d; }
.edit-button i, .delete-button i { margin-right:6px; }

.post-card { display:flex; flex-direction:column; background:#1c1c1e; border-radius:12px; border:1px solid #38383a; overflow:hidden; }
.post-card img { width:100%; height:350px; object-fit:cover; border-bottom:1px solid #38383a; }
.post-content { padding:30px; }
.post-content h2 { margin-top:0; margin-bottom:20px; font-size:26px; font-weight:600; color:<?php echo ($type=='lost')?'#ff453a':'#32d74b'; ?>; }
.post-content p { margin:15px 0; line-height:1.7; font-size:16px; color:#a1a1a6; }
.post-content p strong { color:#e0e0e0; }

.contact-card { margin-top:25px; padding:20px; background:#2c2c2e; border-left:5px solid <?php echo ($type=='lost')?'#ff453a':'#32d74b'; ?>; border-radius:0 8px 8px 0; }
.contact-card h3 { margin-top:0; font-size:18px; display:flex; align-items:center; gap:10px; color:#f5f5f7; }
.contact-card p { margin:8px 0; word-wrap:break-word; color:#a1a1a6; }

.contact-buttons { margin-top:15px; display:flex; gap:10px; flex-wrap:wrap; }
.contact-button { display:inline-flex; align-items:center; gap:8px; padding:10px 20px; font-weight:500; border:none; border-radius:8px; color:white; cursor:pointer; text-align:center; }
.contact-button.line { background:#00c300; }
.contact-button.line:hover { background:#009900; }

@media (min-width:768px){
    .post-card { flex-direction:row; }
    .post-card img { width:45%; height:auto; border-right:1px solid #38383a; border-bottom:none; }
...
<div class="post-card">
    ...
    <div class="post-content">
        ...
        <?php 
        // ปุ่มแก้ไข + ปุ่มลบ สำหรับเจ้าของโพสต์
        ...
        ?>
            ...
            <a href="javascript:void(0);" onclick="confirmDelete('delete_post.php?id=<?= $post['id'] ?>&type=<?= $type ?>');" class="delete-button">
        ...
        <p><strong>สถานะ:</strong> <?php echo htmlspecialchars($post['status']); ?></p>
